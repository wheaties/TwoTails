...
//TODO: This is a test of the conditions upon which "mkDone" works and the by name
//      'y' is handled, e.g. 'y' isn't invoked.
/*class Stinger{
  private def that(f: Int): () => Int = { () => f }

  @mutualrec final def one(x: Int, y: => Int): () => Int = 
    if(0 < x) two(x-1, 1) else that(2)
  @mutualrec final def two(x: Int, y: => Int): ()=> Int = 
    if(0 < x) one(x-1, 1) else that(2)
}*/
...
/*class ByNameTest extends FlatSpec with Matchers{
  ...
  "with byname parameters returning constant literals" should "work" in{
    val hive = new Hive

    hive.one(fourK, 1 + 2) should equal(0)
  }

  "with byname parameters" should "just work" in{
    val yellow = new YelloJacket

    yellow.one(fourK, 5) should equal(5)
  }

  "with byname parameters #2" should "just work" in{
  	val wasp = new Wasp
  ...
  "with byname parameters returning a function" should "just work" in{
    val stinger = new Stinger
    var test = 0

    stinger.one(fourK, { test += 1; 1})() should equal(2)
    test should equal(0)
  }
}*/